...
#### 1.1. Custom function to read and clean each profile ####
...
vertCplot<- function(rowI,DoPlots = FALSE){
  ...
  for (j in Z1:Z2){ 
    ...
    dfInt$ResAreaComp <- dfInt$ResArea..mm.
  ...
  for (k in 1:nrow(dfInt)){
    ...
    dfInt$ResCcomp <- signif(dfInt$ResCcomp,5)
    dfInt$ResAreaComp <- signif(dfInt$ResAreaComp,5)
    #Plots
  if (DoPlots==T){
    ...
    dfInt$ResAreaNo1_2 <- dfInt$ResArea..mm.
...
for (i in 1:length(DataMSCF_List)){
  ...
  DataComb[i,"MeanAreaRaw"] <- mean(df_profile$ResArea..mm.)
  ...
  #DataComb[i,"MeanC_Loess_Raw"] <- mean(df_profile$ResCloessRaw)
